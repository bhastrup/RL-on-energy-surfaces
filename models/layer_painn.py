...
def calc_distance(
    ...
    # Repeat B and n with num_edges in each graph
    B = torch.repeat_interleave(B, splits, dim=0)  # num_edges, 3, 3
    n = torch.repeat_interleave(n, splits, dim=0)  # num_edges, 3, 3
    #print("B")
    #print(B.shape)
    # print(B)
    ...
    left = torch.cross(B, n)
    #print("left")
    #print(left.shape)
    # print(left)

    # Find unit vector normal to B and left as cross product
    up = torch.cross(B, left)
    #print("up")
    #print(up.shape)
    # print(up)
    ...
    diff_B_dot = torch.einsum('ij,ij->i', diff, B.float())
    diff_left_dot = torch.einsum('ij,ij->i', diff, left.float())
    diff_up_dot = torch.einsum('ij,ij->i', diff, up.float())

    B_B_dot = torch.einsum('ij,ij->i', B.float(), B.float())
    left_left_dot = torch.einsum('ij,ij->i', left.float(), left.float())
    up_up_dot = torch.einsum('ij,ij->i', up.float(), up.float())

    #print("diff_B_dot")
    #print(diff_B_dot.shape)
    # print(diff_B_dot)

    x_coef = (diff_B_dot / B_B_dot).unsqueeze(-1)
    y_coef = (diff_left_dot / left_left_dot).unsqueeze(-1)
    ...
    #print("diff_aligned")
    #print(torch.linalg.norm(diff_aligned, dim=1))
    ...
    if return_diff:
        return dist, diff_aligned
    else:
        return dist


def goal_align_edge_diffs(
        edges_diff: torch.Tensor,
        B: torch.Tensor,
        n: torch.Tensor
    ):
    """
    Calculates rotated displacement vectors, thereby representing them in the goal aligned basis

    Args:
        edges_diff: Tensor of shape (num_edges, 3) with Euclidean edge displacement vectors
    
    Returns:
        edges_diff_aligned: Tensor of shape (num_edges, 3) with edge displacement vectors in the goal aligned coordinate system
    """
    ...
    # edges_diff = edges_diff.detach().numpy()
    # B    = B.detach().numpy()
    # left = left.detach().numpy()
    # up   = up.detach().numpy()

    #print(edges_diff)
    #print(edges_diff.detach().numpy())
    
    print(edges_diff.shape)
    print(edges_diff.detach().numpy().shape)

    print(B)
    print(B.shape)
     
    x_coef = (np.dot(edges_diff.detach(), B.detach())/torch.dot(B, B)).unsqueeze(1)
    y_coef = (np.dot(edges_diff, left)/torch.dot(left, left)).unsqueeze(1)
    z_coef = (np.dot(edges_diff, up)/torch.dot(up, up)).unsqueeze(1)

    edges_diff_aligned = x_coef * torch.tensor([1, 0, 0]) + y_coef * torch.tensor([0, 1, 0]) + z_coef * torch.tensor([0, 0, 1])

    #edges_diff_aligned.requires_grad = True

    return edges_diff_aligned
...
def sinc_expansion(input_x: torch.Tensor, expand_params: List[Tuple]):
    ...
    for step_tuple, feat in itertools.zip_longest(expand_params, feat_list):
        ...
        if step_tuple:
            ...
            expanded_list.append(
                torch.sin(n_range * np.pi / cutoff * feat_expanded) / feat_expanded
            )
        else:
            expanded_list.append(torch.unsqueeze(feat, 1))
    return torch.cat(expanded_list, dim=1)
...
class PaiNNInteraction(nn.Module):
    ...
    def forward(
        ...
        """
        # Compute all messages
        edge_vector_normalised = edge_vector / torch.linalg.norm(
            edge_vector, dim=1, keepdim=True
        )  # num_edges, 3
        ...
        #print("new_state_scalar")
        # print(new_state_scalar.shape)
...
class SurfRiderPaiNNReadout(nn.Module):
    """SurfRider readout network. Gives 6-dimensional action space output using both scalar and vectorial nodes"""

    def __init__(self, node_size, action_space_size=6):
        super().__init__()

        # self.agent_atom = agent_atom
        self.node_size = node_size
        self.node_size_half = int(math.ceil(node_size / 2))
        self.linearU = nn.Linear(node_size, node_size, bias=False)
        self.linearV = nn.Linear(node_size, node_size, bias=False)
        self.combined_mlp = nn.Sequential(
            nn.Linear(node_size*4, self.node_size_half),
            nn.SiLU(),
            nn.Linear(self.node_size_half, action_space_size),
        )

    def forward(self, node_state_scalar, node_state_vector):#, agent_atom):
        """
        Args:
            node_state_scalar (tensor): Node states (num_nodes, node_size)
            node_state_vector (tensor): Node states (num_nodes, 3, node_size)
        ...
        Returns:
            Q_value (action_space_size)
        """

        #print("node_state_scalar")
        #print(node_state_scalar.shape)
        #print(node_state_scalar)

        #print("node_state_vector")
        #print(node_state_vector.shape)
        #print(node_state_vector)


        #agent_node_scalar = node_state_scalar[self.agent_atom, :]

        #print("agent_node_scalar")
        #print(agent_node_scalar.shape)
        #print(agent_node_scalar)

        #agent_node_vector = node_state_vector[self.agent_atom, :, :]

        Vs = self.linearU(node_state_scalar)  # node_size
        Uv = self.linearV(node_state_vector)  # 3, node_size

        #print(Vs.shape)
        #print(Uv.shape)
        #print(torch.unsqueeze(Vs, 0).shape)

        mlp_input = torch.cat(
            (torch.unsqueeze(Vs, 1), Uv), dim=1
        )  # num_nodes, node_size*2

        mlp_input = mlp_input.view(-1, 4*self.node_size)
        ...
        Q_value = self.combined_mlp(mlp_input)
