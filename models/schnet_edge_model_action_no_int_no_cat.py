...
class SchnetModel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.readout_mlp_c_half = nn.Sequential(
            nn.Linear(hidden_state_size, math.ceil(hidden_state_size/2)),
            layer.ShiftedSoftplus()
        )

        # Setup readout function
        self.readout_mlp = nn.Linear(math.ceil(hidden_state_size/2), 6)
    ...
    def forward(self, input_dict):
        ...
        node_state_concat = nodes_state_neighbor.view(-1, self.hidden_state_size).float()
	...
	# Remove all invalid outputs correponding to padded edges/nodes
        ...
        graph_output = self.readout_mlp(nodes_C_half_sum)
